...
import { Search, ExternalLink, Lock, RefreshCw, Sparkles, Lightbulb, Calendar } from 'lucide-react'
...
export default function ScholarshipFinder({ profile, hasAllDocs }: { profile: any, hasAllDocs: boolean }) {
  ...
  return (
    <div className="bg-white/3 border border-white/8 rounded-2xl overflow-hidden">
      ...
      {/* Header */}
      <div className="px-4 sm:px-6 py-5 border-b border-white/5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-white font-semibold text-sm truncate">AI Scholarship Finder</h2>
            <p className="text-gray-500 text-xs truncate">Matched to your profile</p>
          </div>
        </div>
        <button
          onClick={findScholarships}
          disabled={loading || !hasAllDocs}
          className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl text-xs font-semibold transition-all shrink-0 ${
            hasAllDocs
        ...
        >
          {!hasAllDocs ? (
            <><Lock className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Upload docs first</span></>
          ) : loading ? (
            <><RefreshCw className="w-3.5 h-3.5 animate-spin" /> <span className="hidden sm:inline">Finding...</span></>
          ) : fetched ? (
            <><RefreshCw className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Refresh</span></>
          ) : (
            <><Search className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Find Scholarships</span></>
          )}
        </button>
      </div>
      ...
      <div className="p-4 sm:p-6">
        ...
        {!loading && scholarships.map((s, i) => (
          <div key={i} className="group p-4 bg-white/3 border border-white/5 rounded-xl hover:bg-white/5 hover:border-white/10 transition-all mb-3 last:mb-0">
            ...
            {s.match_reason && (
              <p className="text-gray-600 text-xs italic mb-3 flex items-start gap-1.5">
                <Lightbulb className="w-3 h-3 shrink-0 mt-0.5" />
                <span>{s.match_reason}</span>
              </p>
            )}
            <div className="flex items-center justify-between gap-2">
              <span className="text-gray-600 text-xs flex items-center gap-1 min-w-0">
                <Calendar className="w-3 h-3 shrink-0" />
                <span className="truncate">{s.deadline}</span>
              </span>
              <a href={s.link} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1 text-blue-400 hover:text-blue-300 text-xs font-medium transition-colors shrink-0">
