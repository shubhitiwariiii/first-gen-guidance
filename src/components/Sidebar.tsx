...
export default function Sidebar({ name, email }: { name: string, email?: string }) {
    ...
    return (
        <aside className="fixed left-0 top-0 h-screen w-[220px] bg-[#0d0d14] border-r border-white/5 flex flex-col z-50">
            ...
            <nav className="px-3 py-4 space-y-0.5">
                ...
                <Link href="/dashboard" className="flex items-center gap-3 px-3 py-2 rounded-lg text-white bg-white/5 border border-white/8 text-sm">
                    <LayoutDashboard className="w-4 h-4 shrink-0 text-blue-400" />
                    <span>Dashboard</span>
                </Link>
                <Link href="/profile" className="flex items-center gap-3 px-3 py-2 rounded-lg text-gray-400 hover:text-white hover:bg-white/5 transition-all text-sm">
