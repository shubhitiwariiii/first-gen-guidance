...
import { GraduationCap, LayoutDashboard, User, LogOut, Menu, X, FileText, Calendar } from 'lucide-react'
...
export default function Sidebar({ name, email, uploadedCount = 0, scholarshipsFound = false, deadlinesCount = 0 }: { 
  ...
  const steps = [
    { label: 'Create profile', done: true, icon: User },
    { label: 'Upload documents', done: uploadedCount >= 4, icon: FileText },
    { label: 'Find scholarships', done: scholarshipsFound, icon: GraduationCap },
    { label: 'Add deadlines', done: deadlinesCount > 0, icon: Calendar },
  ]
